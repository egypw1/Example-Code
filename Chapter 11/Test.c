...
float ADCtoVoltage(const float adcValue, const float VRef);
...
int main()
{
    ...
    const float T0 = 298.15;  //25degC in Kelvin
    const float R0 = 10.0; // resistance at 25degC in kOhms
    const float B = 3975.0; // constant [k]

    // Calculate thermistor temperature in degrees C ( Part b, i,ii,iii & v)

    float adcVoltage = ADCtoVoltage(adcvalueA1, VRef);
    float thermistResistance = ((10.0 * 3.3) / adcVoltage) - 10.0;
    float tempK = 1.0f / ((1.0f / T0) + (1.0 / B) * logf(thermistResistance / R0));
    float thermistorTempC = KelvinToCelsius(tempK);


    // Calculate thermocouple temperature in degrees C ( Part c, i - iv)

    float thermocoupleVoltage = ((adcVoltage - 0.35) / 54.4);     // Calculating thermocouple Voltage (EMF)
...
float ADCtoVoltage(float adcValue, float VRef) {
    float VoltageADC = (adcValue * VRef) / 1024.0; // Converting ADC value to voltage
    return VoltageADC;
}
